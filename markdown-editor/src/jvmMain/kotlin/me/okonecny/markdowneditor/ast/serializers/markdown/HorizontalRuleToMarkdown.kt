...
class HorizontalRuleToMarkdown<D : Any> : VisualNodeSerializer<HorizontalRule, D, AnnotatedString> {
    override fun VisualNodeSerializationContext<D, AnnotatedString>.serializeNode(
        node: VisualNode<HorizontalRule, D>,
        selection: VisualNodeSelection<D>?
    ): AnnotatedString = AnnotatedString(if (selection.hitsNode(node)) "---" else "")
}