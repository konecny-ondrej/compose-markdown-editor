...
class UserMentionToMarkdown<D : Any> : VisualNodeSerializer<UserMention, D, AnnotatedString> {
    override fun VisualNodeSerializationContext<D, AnnotatedString>.serializeNode(
        node: VisualNode<UserMention, D>,
        selection: VisualNodeSelection<D>?
    ): AnnotatedString = AnnotatedString(if (selection.hitsNode(node)) "@" + node.data.username else "")
}