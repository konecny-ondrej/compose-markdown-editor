...
class AnchorToMarkdown<D : Any> : VisualNodeSerializer<Anchor, D, AnnotatedString> {
    override fun VisualNodeSerializationContext<D, AnnotatedString>.serializeNode(
        node: VisualNode<Anchor, D>,
        ...
        if (anchorText.isBlank()) return@buildAnnotatedString

        append("[")
        append(anchorText)
        append("]")
        append("(")
        append(node.data.name)
        append(")")
    }
}